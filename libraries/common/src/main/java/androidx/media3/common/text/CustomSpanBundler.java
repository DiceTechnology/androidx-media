...
/* package */ final class CustomSpanBundler {
  ...
  @Target({TYPE_USE})
  @IntDef({UNKNOWN, RUBY, TEXT_EMPHASIS, HORIZONTAL_TEXT_IN_VERTICAL_CONTEXT, VOICE, TEXT_SHADOW})
  ...
  private static final int TEXT_SHADOW = 101;
  ...
  public static ArrayList<Bundle> bundleCustomSpans(Spanned text) {
    ...
    for (TextShadowSpan span :
        text.getSpans(0, text.length(), TextShadowSpan.class)) {
      Bundle bundle =
          spanToBundle(
              text, span, /* spanType= */ TEXT_SHADOW, /* params= */ span.toBundle());
      bundledCustomSpans.add(bundle);
    }
  ...
  public static void unbundleAndApplyCustomSpan(Bundle customSpanBundle, Spannable text) {
    ...
    switch (customSpanType) {
      ...
      case VOICE:
        ...
        break;
      case TEXT_SHADOW:
        text.setSpan(TextShadowSpan.fromBundle(checkNotNull(span)), start, end, flags);
        break;
      default:
        break;
    }
